...
public sealed class PlayerCircle : IImmutableList<Player>
{
    ...
    private readonly IReadOnlyDictionary<string, int> _nameIndexLookup;
    ...
    private PlayerCircle(IImmutableList<Player> players)
    {
        ...
        _nameIndexLookup = new ReadOnlyDictionary<string, int>(dict);
    }
    ...
    {
        if (!_nameIndexLookup.TryGetValue(name, out int index))
