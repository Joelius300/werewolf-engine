// Licensed to the .NET Foundation under one or more agreements.
// The .NET Foundation licenses this file to you under the MIT license.

using System.Collections.Immutable;
using System.Diagnostics.CodeAnalysis;

namespace WerewolfEngine;

// copied and adapted for ImmutableHashSet from here: https://github.com/dotnet/runtime/blob/5fa18e6ff2a773f85543806b686cb80d5da2f9f6/src/libraries/System.Private.CoreLib/src/System/Collections/Generic/HashSetEqualityComparer.cs
// shame that this doesn't exist yet
public class ImmutableHashSetEqualityComparer<T> : IEqualityComparer<ImmutableHashSet<T>?>
{
    public bool Equals(ImmutableHashSet<T>? x, ImmutableHashSet<T>? y)
    {
        // If they're the exact same instance, they're equal.
        if (ReferenceEquals(x, y))
        {
            return true;
        }

        // They're not both null, so if either is null, they're not equal.
        if (x == null || y == null)
        {
            return false;
        }

        EqualityComparer<T> defaultComparer = EqualityComparer<T>.Default;

        // If both sets use the same comparer, they're equal if they're the same
        // size and one is a "subset" of the other.
        // if (HashSet<T>.EqualityComparersAreEqual(x, y))
        if (x.KeyComparer.Equals(y.KeyComparer))
        {
            return x.Count == y.Count && IsSubsetOfHashSetWithSameComparer(y, x);
        }

        // Otherwise, do an O(N^2) match.
        foreach (T yi in y)
        {
            bool found = false;
            foreach (T xi in x)
            {
                if (defaultComparer.Equals(yi, xi))
                {
                    found = true;
                    break;
                }
            }

            if (!found)
            {
                return false;
            }
        }

        return true;
    }

    public int GetHashCode(ImmutableHashSet<T>? obj)
    {
        int hashCode = 0; // default to 0 for null/empty set

        if (obj != null)
        {
            foreach (T t in obj)
            {
                if (t != null)
                {
                    hashCode ^= t.GetHashCode(); // same hashcode as default comparer
                }
            }
        }

        return hashCode;
    }

    // Equals method for the comparer itself.
    public override bool Equals([NotNullWhen(true)] object? obj) => obj is ImmutableHashSetEqualityComparer<T>;

    public override int GetHashCode() => EqualityComparer<T>.Default.GetHashCode();

    private bool IsSubsetOfHashSetWithSameComparer(ImmutableHashSet<T> a, ImmutableHashSet<T> b)
    {
        foreach (T item in a)
        {
            if (!b.Contains(item))
            {
                return false;
            }
        }

        return true;
    }
}